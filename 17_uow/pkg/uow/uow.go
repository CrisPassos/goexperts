...
func (u *Uow) Do(ctx context.Context, fn func(uow *Uow) error) error {
	...
	u.Tx = tx
	err = fn(u) //princpal linha

	if err != nil {
		errRB := u.Rollback()
		if errRB != nil {
			return errors.New(fmt.Sprintf("uow error: %v, rollback error: %v", err, errRB))
		}
		return err
	}
...
func (u *Uow) Rollback() error {
	if u.Tx == nil {
		return errors.New("there is no active transaction to rollback")
	}
